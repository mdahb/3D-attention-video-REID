...
class Attention_mod(nn.Module):
    ...
    def forward(self, x,flow_a):
        ...
        attention = torch.mean(flow_a, dim=1, keepdim=True)
...
class ResNet(nn.Module):
    ...
    def __init__(self,
       ...
       # pdb.set_trace() 
        ...
        self.attention = Attention_mod()

        self.fc = BNClassifier(2048, num_classes , initialization=True)

        self.fc_mid= BNClassifier(1024, num_classes , initialization=True)
...
#        pdb.set_trace()       
        ...
        x_att_1 = self.attention(x_k,x3d_l3)
        x_att_2 = x_att_1.permute(0,2,1,3,4)        
        x_att = x_att_2.view(b*t,x_p1.size(1),x_p1.size(2),x_p1.size(3)) # 1024,24,8)
                    
        x_att = x_att + x_p1       
        x_p = self.base_a2(x_att)
        x_p = F.avg_pool2d(x_p, x_p.size()[2:])
        x_p = x_p.view(b,t,-1)
        ...
        x_m = F.avg_pool2d(x_att, x_att.size()[2:])
        x_m = x_m.view(b,t,-1)
        x_m=x_m.permute(0,2,1)
        x_fm = F.avg_pool1d(x_m,t)
        x_fm = x_fm.view(b,1024)
        fm,ym = self.fc_mid(x_fm)

...
import models
from models import resnet3d_single_mid
...
def main():
   ...
   # os.environ['CUDA_VISIBLE_DEVICES'] = args.gpu_devices
    ...
    if args.arch=='resnet503d_single_mid':
        model = resnet3d_single_mid.resnet50(num_classes=dataset.num_train_pids, sample_width=args.width, sample_height=args.height, sample_duration=args.seq_len)
...
def get_features(model,imgs,test_num_tracks):
    ...
    for test_imgs in mit.chunked(imgs, test_num_tracks):
        ...
        outputs,  outputs_mid, features1, features_mid = model(current_test_imgs)
        features1 = features1.view(num_current_test_imgs, -1)
        features_mid = features_mid.view(num_current_test_imgs, -1)
        features = torch.cat((features1,features_mid),1)
